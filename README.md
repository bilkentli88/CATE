...
- class-logit score `w_c_hat^T e_t`;
- margin-logit score `(w_c_hat - w_c_prime)^T e_t`;
- gradient saliency;
- input x gradient;
...
- input time series;
- class-logit evidence score `w_c_hat^T e_t`;
- margin-logit evidence score `(w_c_hat - w_c_prime)^T e_t`.
...
u = sum_t alpha_t z_t = sum_t e_t
```

The classifier-aware scores used in the paper are:

```text
s_class(t)  = w_c_hat^T e_t
s_margin(t) = (w_c_hat - w_c_prime)^T e_t
```
